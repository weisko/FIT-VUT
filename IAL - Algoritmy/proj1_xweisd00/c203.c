...
*/
	...
	else{						//spravny parameter 
		for(int i = 0; i < QUEUE_SIZE; i++){
			q->arr[i] = '*';	//naplni pole znakom '*'
		}
		q->f_index = 0;			//nastavenie indexu prveho prvku na 0
		q->b_index = 0;			//nastavenie indexu prvej volnej pozicie na 0
	}
}
...
*/
	return((++index) % (QUEUE_SIZE)); //o jedno sa posunie na konci prejde na zaciatok
...
*/
	return((q->f_index == q->b_index) ? 1 : 0);	//ak sa prvy prvok rovna poslednemu
}
...
*/
	return((nextIndex(q->b_index) == q->f_index) ? 1 : 0); //fronta je plna ak sa prva volna pozicia bude rovnat prvemu prvku
}
...
*/
	if(queueEmpty(q) == 0){ 		//ak nieje fronta prazdna
		*c = q->arr[q->f_index]; 	//ulozime znak zo zaciatku fronty
	}
	else{							//aj je fronta prazdna
		queueError(QERR_FRONT);		//vypis erroroveho hlasenia
	}
}
...
*/
	if(queueEmpty(q) == 0){ 				//aj nieje fronta prazdna
...
*/
	if(queueEmpty(q) == 0){ 	//ak nie je fronta prazdna
		queueFront(q, c); 		//najprv vratime znak
		queueRemove(q); 		//potom odstranime
	}
	else{ 						//ak je fronta prazdna
		queueError(QERR_GET);	//vypis erroroveho hlasenia
	}
}
...
*/
	if(queueFull(q) == 0){ 						//ak fronta nieje plna
		q->arr[q->b_index] = c; 				//vlozi sa znak do frony
		q->b_index = nextIndex(q->b_index); 	//posunieme sa
	}
	else{ 										//ak je fronta plna
		queueError(QERR_UP);					//bvypis erroroveho halsenia
	}
}
